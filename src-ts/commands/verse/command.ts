import {
  SlashCommandBuilder,
  ChatInputCommandInteraction,
  SlashCommandIntegerOption,
  SlashCommandStringOption,
} from "discord.js";
import { getCachedEmbed, setCache } from "../../utility/db";
import { isValidScriptureCode, isValidVocalization } from "../../utility/func";
import { Command, TVocalizationKey } from "../../utility/types";
import { SCRIPTURE_DATA } from "../../utility/util";
import { fetchVerse, getEmbedForVerse } from "./utils/functions";

export const command: Command = {
  cooldown: 5,
  data: new SlashCommandBuilder()
    .setName("verse")
    .setDescription(
      "Fetches a specific verse from the selected scripture (e.g. Torah), including its translation and vocalization options."
    )
    .addStringOption((option: SlashCommandStringOption) =>
      option
        .setName("scripture_code")
        .setDescription("Which scripture? e.g. t for Torah etc.")
        .addChoices({ name: "Torah", value: "t" })
        .setRequired(true)
    )
    .addIntegerOption((option: SlashCommandIntegerOption) =>
      option
        .setName("section_number")
        .setDescription("Section number.")
        .setRequired(true)
    )
    .addIntegerOption((option: SlashCommandIntegerOption) =>
      option
        .setName("chapter_number")
        .setDescription("Chapter number.")
        .setRequired(true)
    )
    .addIntegerOption((option: SlashCommandIntegerOption) =>
      option
        .setName("verse_number")
        .setDescription("Verse number.")
        .setRequired(true)
    )
    .addIntegerOption((option: SlashCommandIntegerOption) =>
      option
        .setName("translation")
        .setDescription(
          "(Optional) Identification number of Translation you want."
        )
        .addChoices({
          name: "The Contemporary Torah",
          value: 1,
        })
        .setRequired(false)
    )
    .addStringOption((option: SlashCommandStringOption) =>
      option
        .setName("vocalization")
        .setDescription(
          "(Optional) Vocalization of text. If not available, then default vocalization will be displayed."
        )
        .addChoices(
          {
            name: "Usual",
            value: "usual",
          },
          { name: "Simplified", value: "simplified" },
          { name: "Without Vowel", value: "withoutVowel" }
        )
        .setRequired(false)
    )
    .toJSON(),

  execute: async (interaction: ChatInputCommandInteraction) => {
    const scriptureCodeParam = interaction.options.getString(
      "scripture_code",
      true
    );

    if (!isValidScriptureCode(scriptureCodeParam)) {
      await interaction.reply({
        content: `There is no scripture found for code: ${scriptureCodeParam}`,
      });
      return;
    }

    const scriptureCode = scriptureCodeParam;

    const sectionNumber = interaction.options.getInteger(
      "section_number",
      true
    );
    const chapterNumber = interaction.options.getInteger(
      "chapter_number",
      true
    );
    const verseNumber = interaction.options.getInteger("verse_number", true);
    const translationId = interaction.options.getInteger("translation", false);

    const vocalizationKeyParam = interaction.options.getString(
      "vocalization",
      false
    );
    const vocalizationKey: TVocalizationKey =
      vocalizationKeyParam && isValidVocalization(vocalizationKeyParam)
        ? vocalizationKeyParam
        : "usual";

    const cacheKey = `verse-${scriptureCode}-${sectionNumber}-${chapterNumber}-${verseNumber}-${vocalizationKey}-${
      translationId ?? -1
    }`;

    const cachedEmbed = await getCachedEmbed(cacheKey);

    if (cachedEmbed) {
      await interaction.reply({ embeds: [cachedEmbed] });
      return;
    }

    const details = SCRIPTURE_DATA[scriptureCode];
    const scriptureNumber = details.getNumber();

    const verse = await fetchVerse(
      scriptureNumber,
      sectionNumber,
      chapterNumber,
      verseNumber
    );

    if (!verse) {
      await interaction.reply(
        `📥 Something went unexpectedly wrong. If the issue persists, please report it.`
      );
      return;
    }

    const embed = getEmbedForVerse(verse, translationId, vocalizationKey);
    const SECONDS_IN_TEN_DAYS = 60 * 60 * 24 * 10;

    await setCache(cacheKey, embed, SECONDS_IN_TEN_DAYS);
    await interaction.reply({ embeds: [embed] });
  },
};

export default command;
